...
pf = Path(r"C:\Users\kando\AppData\Local\Temp\famidash_pf_trace_untitled_20260517_175050_795.csv")
...
for f in sorted(pf_rows):
    ...
    if px != mx or py != my or pv != mv:
        if first_any is None:
            first_any = (f, f + off, px, mx, py, my, pv, mv)
...
if first_any:
    ...
    for f in range(f0 - 2, f0 + 6):
        ...
        if int(pr['mode']) != 6 or int(mr['gamemode']) != 6:
            continue
        print(f, f + off, 'x', pr['X_px'], mr['px'], 'y', pr['Y_px'], mr['py'], 'vy', int(pr['VelY_fixed'], 16) & 0xFFFF, int(mr['vel_y']) & 0xFFFF)

...
namespace FamidashEditor
{
    ...
    public class PathfinderEngine
    {
        ...
        [ThreadStatic] private static int? _dualP2FrameEntryVelXOverride;
        ...
        // -- Debug logging ---------------------------------------------------
#if !DISABLE_DEBUG_LOGGING
        // These paths are lazily stamped on first use (or by TraceFrameOpen,
        // whichever comes first) so they pick up LevelName set by the host
        // immediately after construction.  Empty string means "not yet
        // stamped".
        ...
        {
            if (pfDebugLogPath.Length != 0) return;
            try
            {
                string tag = SanitizeLevelTag(LevelName ?? "");
                string ts = System.DateTime.UtcNow.ToString("yyyyMMdd_HHmmss_fff");
                string tmp = System.IO.Path.GetTempPath();
                pfDebugLogPath    = System.IO.Path.Combine(tmp, $"famidash_pf_debug_{tag}_{ts}.txt");
                pfOrbDebugLogPath = System.IO.Path.Combine(tmp, $"famidash_pf_orb_debug_{tag}_{ts}.log");
            }
            catch
            {
                // Last-ditch fallback so logging still works if something is wrong with LevelName.
                string ts = System.DateTime.UtcNow.ToString("yyyyMMdd_HHmmss_fff");
                string tmp = System.IO.Path.GetTempPath();
                pfDebugLogPath    = System.IO.Path.Combine(tmp, $"famidash_pf_debug_unknown_{ts}.txt");
                pfOrbDebugLogPath = System.IO.Path.Combine(tmp, $"famidash_pf_orb_debug_unknown_{ts}.log");
            }
        }

        // Tags whose PfLog() lines also get mirrored to the per-run orb debug log.
        private static readonly string[] _orbLogTagPrefixes = new[]
        {
            "[ORB", "[DASH_ORB", "[DECIDE_ORB", "[PAD", "[BLUE_PAD",
            "[GREEN_PAD", "[YELLOW_PAD", "[GRAV_PAD", "[ORB_TRACK"
        };

        private static bool MatchesOrbTag(string msg)
        {
            for (int i = 0; i < _orbLogTagPrefixes.Length; i++)
                if (msg.StartsWith(_orbLogTagPrefixes[i], System.StringComparison.Ordinal))
                    return true;
            return false;
        }
        ...
        private void PfLog(string msg)
        {
            ...
            try
            {
                ...
                if (MatchesOrbTag(msg))
                    System.IO.File.AppendAllText(pfOrbDebugLogPath, line + System.Environment.NewLine);
            }
            catch { }
        }
        ...
        /// <summary>Path to the pathfinder orb debug log (in %TEMP%). Empty if logging compiled out.</summary>
        ...
        /// <summary>NES spawn Y config lo byte (optional).</summary>
        ...
        private void ApplyNesIntroFreezePrestep(ref SimState s)
        {
            ...
            int gravity = SharedPhysics.GetCubeGravity(s.Mini);
            if (s.GravFlipped) gravity = -gravity;

            s.VelY_fixed += gravity;
            s.Y_fixed += s.VelY_fixed;
            s.X_fixed += s.VelX_fixed;
#if !DISABLE_DEBUG_LOGGING
            PfLog($"[INTRO_PRESTEP] X=0x{s.X_fixed:X4} Y=0x{s.Y_fixed:X4} VelY=0x{s.VelY_fixed:X4} gravity=0x{gravity:X4} mode={s.GameMode}");
#endif
        }
        ...
        {
            // NES stores activesprites_y as raw 16-bit LINEAR pixels (see
            // export_levels.py: y & 0xFF, (y>>8) & 0xFF).  Then:
            //   target_scroll_y = activesprites_y - PORTAL_TO_TOP_DIFF (linear-16bit subtract)
            //   if (low_byte(target_scroll_y) >= 0xF0) target_scroll_y += 0x10
            // The camera converges when raw scroll_y == raw target.  Physical
            // NES position = hi*240 + lo (NT linearization).
            int rawTarget = portalWorldY_px - PORTAL_TO_TOP_DIFF_PX;
            int nesLinear = rawTarget + _nesCoordOffset;
            if (nesLinear < 0x100) // high byte = 0 → no NT distortion
            ...
            int maxCamPF = NesMaxCamY_px();
            if (effectivePF > maxCamPF) effectivePF = maxCamPF;
            return Math.Max(0, effectivePF << 8);
        }

        private int NesMaxCamY_px()
        {
            // NES camera cannot scroll below (mapHeight-15)*16 rows or 719-nesCoordOffset.
            // Matches release: Math.Max(0, Math.Min((mapHeight-15)*16, 719-_nesCoordOffset))
        ...
        // Sanitize a level/TMX path into a filename-safe tag (mirrors
        // PfTrace.SanitizeName so all per-run files share the same suffix).
        ...
        {
            // Restamp all per-run debug paths with the current LevelName so
            // each run produces a stable, identifiable file set:
            //   famidash_pf_debug_<level>_<ts>.txt
            //   famidash_pf_orb_debug_<level>_<ts>.log
            //   famidash_pf_trace_<level>_<ts>.csv
            // (And PfTrace.Open below produces famidash_pf_trace_FULL_...)
            // Force a fresh stamp at the start of each run so multiple runs
            // in the same process get distinct file sets.
            ...
            try
            {
                ...
                string ts = System.DateTime.UtcNow.ToString("yyyyMMdd_HHmmss_fff");
                string tmp = System.IO.Path.GetTempPath();
                _frameTracePath = System.IO.Path.Combine(tmp, $"famidash_pf_trace_{tag}_{ts}.csv");
            }
            catch { }
            // Pre-create the orb-debug file so the per-run file set always
            // contains it even if no orb-tag PfLog fires during the run.
            try { System.IO.File.WriteAllText(pfOrbDebugLogPath, $"# pf orb debug {System.DateTime.UtcNow:o} level={LevelName}{System.Environment.NewLine}"); }
            catch { }
            try
            {
            ...
            // Open the comprehensive PfTrace FULL log alongside the CSV.
            // Activated by env var FAMIDASH_PF_TRACE=1 (read in PfTrace static
            // ctor) or by caller setting PfTrace.Enabled=true.  Produces
            // %TEMP%\famidash_pf_trace_FULL_<sanitized-level>_<ts>.log.
            try { if (PfTrace.Enabled) PfTrace.Open(LevelName ?? "unknown"); }
            catch { }
        }
        ...
        private void TraceFrame(int frame, ref SimState s, bool input, bool alive)
        {
            ...
            try
            {
                // X_px / Y_px output as hitbox-CENTER (top + 8) to match
                // Mesen trace conventions used by parity comparison tools.
                int traceX_px = (s.X_fixed >> 8) + 8;
                bool camFollowsY = (!s.DualActive) &&
                    (s.GameMode == 0 || s.GameMode == 4 || s.GameMode == 8 || s.GameMode == 9 || s.NoCamLockForced);
                int traceY_px = camFollowsY
                    ? (s.CameraY_fixed >> 8) + ((s.Y_fixed - s.CameraY_fixed) >> 8) + 8
                    : (s.Y_fixed >> 8) + 8;
                _traceWriter.WriteLine($"{frame},0x{s.X_fixed:X},0x{s.Y_fixed:X},0x{s.VelY_fixed:X},{(input ? 1 : 0)},{(alive ? 1 : 0)},{traceX_px},{traceY_px},{(s.OnGround ? 1 : 0)},{s.CameraY_fixed >> 8},{s.TargetCameraY_fixed >> 8}");
            }
            catch { }
        }
        ...
        private struct SimState
        {
            ...
            public bool GravFlippedAtFrameStart;    // captured at start of ProcessSprites (before any portal flips)
            public bool OrbUseFrameStartGravitySign; // true when gravity portal + non-blue orb coincide in same frame
            public bool Mini;
            ...
            Dictionary<int, (int offsetX, int offsetY)>? spritePixelOffsets = null)
        {
            ...
            // Diagnostic: dump config hash so pf-test vs editor divergences are detectable
            {
                ...
                _log.Flush();
#if !DISABLE_DEBUG_LOGGING
                try { EnsureDebugPathsStamped(); System.IO.File.AppendAllText(pfDebugLogPath, diagMsg + System.Environment.NewLine); } catch { }
#endif
            }
        }
        ...
        public void ReplayInputSequence(int startX_px, int startY_px,
            int startSpeedUiIndex, int startGameMode, bool startGravFlipped, bool startMini,
            IReadOnlyList<bool> inputSequence, int preRollFrames = 0, System.IO.TextWriter? output = null)
        {
            _log = Verbose ? Console.Error : System.IO.TextWriter.Null;
            ...
            PathPoints = new List<(int x, int y)>();
            Path2Points = new List<(int x, int y)>();
            Inputs = new List<bool>();
            Success = false;
            ...
            int startIndex = Math.Max(0, Math.Min(preRollFrames, inputSequence.Count));
            ...
            try
            {
                for (int i = startIndex; i < inputSequence.Count; i++)
                {
                    int frame = i - startIndex;
                    _frameCounter = frame;
                    bool input = inputSequence[i];
                    Inputs.Add(input);
                    ...
                    TraceFrame(frame, ref state, Inputs[Inputs.Count - 1], alive);
                    ...
                    replayedFrames = frame + 1;
                    if (endLevel)
                    {
                        reachedEnd = true;
                        Success = true;
                        ResultMessage = $"TAS replay completed in {replayedFrames} frames";
                        break;
                    }
                    if (!alive)
                    {
                        died = true;
                        Success = false;
                        ResultMessage = $"TAS replay died at frame {frame} (input line {i})";
                        break;
                    }
                }
            }
            ...
            if (!reachedEnd && !died)
                ResultMessage = $"TAS replay exhausted {replayedFrames} frames without ending level";
                            ...
                            int startGameMode, bool startGravFlipped, bool startMini)
        {
...
#if !DISABLE_DEBUG_LOGGING
            void BfsLog(string msg) { try { EnsureDebugPathsStamped(); System.IO.File.AppendAllText(pfDebugLogPath, "[BFS] " + msg + System.Environment.NewLine); } catch { } }
            ...
            catch (Exception ex)
            {
                ...
                Success = false;
#if !DISABLE_DEBUG_LOGGING
                try { EnsureDebugPathsStamped(); System.IO.File.AppendAllText(pfDebugLogPath, $"[BFS] CRASHED: {ex.GetType().Name}: {ex.Message}\n{ex.StackTrace}\n"); } catch { }
#endif
            }
        ...
        private bool StepFrame(ref SimState s, bool input, out bool endLevel)
        {
            ...
            // -- PfTrace per-frame context (no-op fast-path when disabled) --
            // cur=0 for P1, cur=1 when this StepFrame call is the recursive P2
            // pass inside dual mode (guarded by _dualP2Guard).
            ...
            int frameEntryVelX_fixed = (_dualP2Guard && _dualP2FrameEntryVelXOverride.HasValue)
                ? _dualP2FrameEntryVelXOverride.Value
                : s.VelX_fixed;
            ...
            else if (s.GameMode == 6) // Wave mode
            {
                ...
                int baseVelY = s.Mini ? (frameEntryVelX_fixed << 1) : frameEntryVelX_fixed;
            ...
            if (s.GameMode == 0 || s.GameMode == 1 || s.GameMode == 2 || s.GameMode == 3 || s.GameMode == 4 || s.GameMode == 5 || s.GameMode == 6 || s.GameMode == 7 || s.GameMode == 8 || s.GameMode == 9 || s.GameMode == 10)
            {
                ...
                if (s.SlopeWasOnCounter == 0 && s.GameMode != 6 && s.GameMode != 10)
                {
                    ...
                    var (nudge, _) = SharedPhysics.GetForwardSlopeNudge(_collisionMap,
            ...
            {
                // NES do_the_scroll_thing/process_y_scroll runs once per frame after P1,
                // before P2 is loaded. P2 should not apply another camera/scroll pass.
                if (!_dualP2Guard)
                {
            ...
            if (s.DualActive && !_dualP2Guard)
            {
                ...
                int? previousP2FrameEntryVelXOverride = _dualP2FrameEntryVelXOverride;
                _dualP2FrameEntryVelXOverride = frameEntryVelX_fixed;
                _dualP2Guard = true;
                bool p2EndLevel = false;
                bool p2Alive;
                try
                {
                    p2Alive = StepFrame(ref s, p2Input, out p2EndLevel);
                }
                finally
                {
                    _dualP2Guard = false;
                    _dualP2FrameEntryVelXOverride = previousP2FrameEntryVelXOverride;
                }
                foreach (var idx in _p1OrbIndicesThisFrame!)
        ...
        /// Matching NES gamemode_ship.h, including signed table entries for inverted gravity.
        /// </summary>
        private void ShipGravityAndThrust(ref SimState s, bool holding, out bool died)
        {
            ...
            bool falling = s.GravFlipped ? (s.VelY_fixed < 0) : (s.VelY_fixed > 0);

            int gravityMagnitude;
            if (holding && falling)
                gravityMagnitude = ShipGravityHoldFall(s.Mini);
            else if (holding)
                gravityMagnitude = ShipGravityBase(s.Mini);
            else if (!falling)
                gravityMagnitude = ShipGravityAfterHold(s.Mini);
            else
                gravityMagnitude = ShipGravity(s.Mini);

            int gravity = gravityMagnitude * (s.GravFlipped ? -1 : 1);
        ...
        private bool ProcessSprites(ref SimState s, int currentX_px, out bool orbHitThisFrame)
        {
            ...
            // Capture gravity sign before any portal can flip it this frame.
            // Used by ApplyOrbSprite when a gravity portal and orb coincide.
            ...
            // Wave and snake (modes 6 and 10) use a fixed 8×8 hitbox with offset 4,
            // not the generic cube hitbox. Matches NES sprite_collide() which reads
            // currplayer_height=8, currplayer_width=8, currplayer_y_off=4 for these modes.
            bool isWaveSnakeMode = (s.GameMode == 6 || s.GameMode == 10);
            int hbW = isWaveSnakeMode ? 8 : GetHitboxW(s.Mini);
            int hbH = isWaveSnakeMode ? 8 : GetHitboxH(s.Mini);
            int hbOffY = isWaveSnakeMode ? 4 : GetHitboxOffsetY(s.GameMode, s.Mini, s.GravFlipped);
            int playerTop = playerY_px + hbOffY;
            int playerBottom = playerTop + hbH;
            ...
            for (int _si = sprStart; _si < sprLen; _si++)
            {
                    ...
                    IsMiniGrowthPortal(sid) || IsEndLevel(sid))
                {
                    ...
                    if (hit)
                    {
                        ...
                        if (applied)
                            s.ProcessedSprites.Add(sp.Index);
                        // When a gravity portal fires, check for coincident non-blue orbs
                        // at the same sprite X position. If found, the orb must use the
                        // frame-start gravity sign (before the portal flipped it).
                        // Matches NES sprite_collide() which scans all sprites in one pass.
                        if (IsGravityPortal(sid) && applied)
                        {
                            for (int _csi = sprStart; _csi < sprLen; _csi++)
                            {
                                ref readonly var csp = ref sprArr[_csi];
                                if (csp.HitRight < currentX_px) continue;
                                if (csp.AnchorX_px - TILE > playerRight + TILE) break;
                                if (csp.AnchorX_px != sp.AnchorX_px) continue;
                                if (s.ProcessedSprites.Contains(csp.Index)) continue;
                                int csid = csp.SpriteId;
                                if (IsYellowOrb(csid) || IsYellowOrbBigger(csid) || IsYellowOrbSmaller(csid) ||
                                    IsPinkOrb(csid) || IsRedOrb(csid) || IsBlackOrb(csid))
                                {
                                    bool cxOvlp = !((playerRight) < csp.HitLeft || csp.HitRight < nesX);
                                    bool cyOvlp = !((playerBottom) < csp.HitTop || csp.HitBottom < playerTop);
                                    if (cxOvlp && cyOvlp)
                                    {
...
#if !DISABLE_DEBUG_LOGGING
                    if (IsYellowPad(sid) && _frameCounter >= 2378 && _frameCounter <= 2383)
                    {
                        PfLog($"[YPAD_CHECK] f={_frameCounter} idx={sp.Index} sid=0x{sid:X2} player=({padLeft},{playerTop})-({padRight},{playerBottom}) sprite=({sp.HitLeft},{sp.HitTop})-({sp.HitRight},{sp.HitBottom}) xOvlp={xOverlap} yOvlp={yOverlap} gravFlipped={s.GravFlipped} mode={s.GameMode}");
                    }
#endif
        ...
        /// </summary>
        private static bool IsWaveOrSnakeMode(int mode) => mode == 6 || mode == 10;

        private static int NesSignedHalf(int value) => value >> 1;

        private static string ApplyGameModePortalVelocityRule(ref SimState s, int sid, int prevMode, int newMode)
        {
            bool prevWaveSnake = IsWaveOrSnakeMode(prevMode);
            switch (sid)
            {
                case 0x01:
                case 0x02:
        ...
        private bool ApplyPortalSprite(ref SimState s, int sid)
        {
            ...
            if (IsGameModePortal(sid))
            {
                ...
                if (mode >= 0 && mode != s.GameMode)
                {
                    ...
                    string velRule = ApplyGameModePortalVelocityRule(ref s, sid, prevMode, mode);
#if !DISABLE_DEBUG_LOGGING
                    PfLog($"[PORTAL_GAMEMODE] sid=0x{sid:X2} mode {prevMode} -> {mode} VelY {velRule}: 0x{prevVelY:X4} -> 0x{s.VelY_fixed:X4}");
#endif
                    ...
                    // Clear slope state that carried over from previous mode.
                    // Without this, stale slope counters leak into ship/UFO eject
                    // and can trigger spurious floor-lock on the first ship frame.
            ...
            else if (IsGravityPortal(sid))
            {
                ...
                {
                    int newVelY = NesSignedHalf(s.VelY_fixed);
                    s.GravFlipped = true;
                    s.GravMul = -1;
#if !DISABLE_DEBUG_LOGGING
                    PfLog($"[PORTAL_GRAV_FLIP] REVERSED! VelY halved: 0x{s.VelY_fixed:X4} -> 0x{newVelY:X4}");
#endif
                    s.VelY_fixed = newVelY;
                    s.WasZeroedByCollision = false;
                    return true;
                }
                else if (!isReverse && s.GravFlipped)
                {
                    ...
                    s.GravMul = 1;
#if !DISABLE_DEBUG_LOGGING
                    PfLog($"[PORTAL_GRAV_FLIP] NORMAL! VelY halved: 0x{s.VelY_fixed:X4} -> 0x{newVelY:X4}");
#endif
                    s.VelY_fixed = newVelY;
                    s.WasZeroedByCollision = false;
                    return true;
...
#endif
            // When a gravity portal and a non-blue orb coincide in the same frame,
            // the orb must use the gravity direction from the START of the frame
            // (before the portal flipped it). Matches NES sprite_collide() ordering.
            int orbGravSign = (s.OrbUseFrameStartGravitySign ? s.GravFlippedAtFrameStart : s.GravFlipped) ? 1 : -1;
        ...
        {
            var (hit, ceilingBottomY, spikeDeath, _) = SharedPhysics.CheckCeiling(_collisionMap, collX, collY, collW, collH);
            return (hit, ceilingBottomY, spikeDeath);
        }
        ...
        private static void PfUpdateSlopeCounters(ref SimState s)
        {
            SharedPhysics.UpdateSlopeCounters(ref s.SlopeWasOnCounter, ref s.SlopeType,
                ref s.VelY_fixed, ref s.Y_fixed, s.GameMode, s.GravFlipped, s.Mini, ref s.LastSlopeType);
        }

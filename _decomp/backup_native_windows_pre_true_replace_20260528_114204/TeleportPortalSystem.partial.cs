                ...
                for (int _si = 0; _si < nonEmptySpriteIndices.Length; _si++)
                {
                    ...
                    if (SpriteIntersectsPlayer(idx, sid, playerLeft_px, playerRight_px, playerTop_px, playerBottom_px))
                    {
                        ...
                        if (foundExit)
                        {
                            ...
                            // NES sprite_loading.h:976 `high_byte(currplayer_y) = teleport_output;`
                            // — only cpy_high is overwritten; cpy_low is preserved.  Preserve
                            // Y_fixed.low so the borrow pattern of subsequent gravity steps
                            // matches NES (parity fix for silentcircles ball mode false ceiling).
                            playerY_fixed = (exitY_px << 8) | (playerY_fixed & 0xFF);

...
public static class TmxHandler
{
	...
	private static bool IsTriggerSprite(int spriteIdx)
	{
		...
		return spriteIdx == 15 || spriteIdx == 111 || (spriteIdx >= 112 && spriteIdx <= 116) || spriteIdx == 125 || spriteIdx == 127 || (spriteIdx >= 128 && spriteIdx <= 239) || (spriteIdx >= 240 && spriteIdx <= 245);
	}
	...
	public static TmxLevel LoadTmx(string filePath, bool useLegacyTriggerOffset = false)
	{
		...
		string text = null;
		string text2 = null;
		List<XElement> list2 = xElement.Elements("tileset").ToList();
		List<int> list3 = new List<int>();
		List<int> list4 = new List<int>();
		foreach (XElement item16 in list2)
		{
			int valueOrDefault2 = ((int?)item16.Attribute("firstgid")).GetValueOrDefault();
			string a = (string?)item16.Attribute("name");
			XElement xElement2 = item16.Element("image");
			if (xElement2 == null)
			{
				continue;
			}
		...
		List<(int, int, int, int, int, bool)> list6 = new List<(int, int, int, int, int, bool)>();
		List<XElement> list7 = xElement.Elements("layer").ToList();
		foreach (XElement item17 in list7)
		{
			bool flag = ((string?)item17.Attribute("name"))?.Equals("SP", StringComparison.OrdinalIgnoreCase) ?? false;
			XElement xElement3 = item17.Element("data");
			if (xElement3 == null)
			{
				continue;
			}
			string text4 = (string?)xElement3.Attribute("encoding");
			if (text4 == "csv")
			{
				string text5 = xElement3.Value.Trim();
				int[] array2 = (from s in text5.Split(new char[3] { ',', '\n', '\r' }, StringSplitOptions.RemoveEmptyEntries)
				...
				for (int num2 = 0; num2 < Math.Min(array2.Length, num); num2++)
				{
					...
					bool flag2 = false;
					foreach (int item18 in list3)
					{
						if (num3 < item18 || num3 >= item18 + 256)
						{
							continue;
						}
						int num4 = num3 - item18;
						int num5 = num2 / width;
						int num6 = num2 % width;
						int item = num6;
						int item2 = num5;
						bool flag3 = !useLegacyTriggerOffset && IsTriggerSprite(num4);
						if (flag3)
						{
							num6 -= 10;
							if (num6 < 0)
						...
						list6.Add((num4, num6, num5, item, item2, flag3));
						flag2 = true;
						break;
					}
					if (flag2)
					...
					foreach (int item19 in list4)
					{
						if (num3 >= item19 && num3 < item19 + 256)
						{
							array[num2] = num3 - item19;
							flag2 = true;
							break;
						}
					}
		...
		List<(int, int, int, int, int, bool)> list8 = list6.Where<(int, int, int, int, int, bool)>(((int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) s) => !s.isTrigger).ToList();
		List<(int, int, int, int, int, bool)> list9 = list6.Where<(int, int, int, int, int, bool)>(((int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) s) => s.isTrigger).ToList();
		list8.Sort(delegate((int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) tuple, (int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) b)
		{
			int num20 = tuple.y.CompareTo(b.y);
			return (num20 != 0) ? num20 : tuple.x.CompareTo(b.x);
		});
		list9.Sort(delegate((int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) tuple, (int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) b)
		{
			int num20 = tuple.x.CompareTo(b.x);
			return (num20 != 0) ? num20 : tuple.y.CompareTo(b.y);
		});
		foreach (var item20 in list8)
		{
			int item3 = item20.Item1;
			int item4 = item20.Item2;
			int item5 = item20.Item3;
			int item6 = item20.Item4;
			int item7 = item20.Item5;
			bool item8 = item20.Item6;
			...
			if (sprites[num7] != -1)
			{
				...
				bool flag4 = IsInteractiveSprite(num8);
				bool flag5 = IsInteractiveSprite(item3);
				if (flag4 && !flag5)
				{
					list5.Add($"NORMAL Sprite 0x{item3:X2} (deco) at TMX({item6},{item7}) SKIPPED — would overwrite interactive sprite 0x{num8:X2} at ({item4},{item5})");
					continue;
				}
				list5.Add($"NORMAL Sprite 0x{item3:X2} at TMX({item6},{item7}) OVERWRITES existing sprite 0x{num8:X2} at ({item4},{item5})");
			}
			sprites[num7] = item3;
		}
		...
		{
			int num20 = Math.Max(0, Math.Min(maxPlayableRow, row));
			int num21 = num20 * width + col;
			if (sprites[num21] == -1)
			{
				sprites[num21] = spriteId;
				return num20;
			}
			int spriteIdx = sprites[num21];
			if (!IsTriggerSprite(spriteIdx))
			{
				for (int i = 0; i <= maxPlayableRow; i++)
				{
					...
					{
						int num22 = row - i;
						if (num22 >= 0 && num22 <= maxPlayableRow)
						{
							int num23 = num22 * width + col;
							if (sprites[num23] == -1)
							{
								sprites[num23] = spriteId;
								return num22;
							}
						}
						if (i != 0)
						{
							int num24 = row + i;
							if (num24 >= 0 && num24 <= maxPlayableRow)
							{
								int num25 = num24 * width + col;
								if (sprites[num25] == -1)
								{
									sprites[num25] = spriteId;
									return num24;
								}
							}
						}
					...
					{
						int num26 = row + i;
						if (num26 >= 0 && num26 <= maxPlayableRow)
						{
							int num27 = num26 * width + col;
							if (sprites[num27] == -1)
							{
								sprites[num27] = spriteId;
								return num26;
							}
						}
						if (i != 0)
						{
							int num28 = row - i;
							if (num28 >= 0 && num28 <= maxPlayableRow)
							{
								int num29 = num28 * width + col;
								if (sprites[num29] == -1)
								{
									sprites[num29] = spriteId;
									return num28;
								}
							}
						}
			...
			int item15 = (col << 16) | num20;
			if (pushVisited.Contains(item15))
			{
				return -1;
			}
			pushVisited.Add(item15);
			int arg2 = sprites[num21];
			if (preferUpFirst)
			{
				for (int j = 1; j <= maxPlayableRow; j++)
				{
					...
					if (num30 >= 0)
					{
						int num31 = PlaceTriggerWithPush(col, num30, arg2, preferUpFirst);
						if (num31 >= 0)
						{
							sprites[num21] = spriteId;
							pushVisited.Remove(item15);
							return num20;
						}
					}
					int num32 = num20 + j;
					if (num32 <= maxPlayableRow)
					{
						int num33 = PlaceTriggerWithPush(col, num32, arg2, preferUpFirst);
						if (num33 >= 0)
						{
							sprites[num21] = spriteId;
							pushVisited.Remove(item15);
			...
			else
			{
				...
				{
					int num34 = num20 + k;
					if (num34 <= maxPlayableRow)
					{
						int num35 = PlaceTriggerWithPush(col, num34, arg2, preferUpFirst);
						if (num35 >= 0)
						{
							sprites[num21] = spriteId;
							pushVisited.Remove(item15);
					...
					if (num36 >= 0)
					{
						int num37 = PlaceTriggerWithPush(col, num36, arg2, preferUpFirst);
						if (num37 >= 0)
						{
							sprites[num21] = spriteId;
							pushVisited.Remove(item15);
		...
		foreach (var item21 in list9)
		{
			int item9 = item21.Item1;
			int item10 = item21.Item2;
			int item11 = item21.Item3;
			int item12 = item21.Item4;
			int item13 = item21.Item5;
			bool item14 = item21.Item6;
			int num9 = Math.Max(0, Math.Min(width - 1, item10));
			int num10 = -1;
			bool arg = false;
			try
			{
				int num11 = Math.Max(0, valueOrDefault - 1 - 8);
				int num12 = Math.Max(0, valueOrDefault - 2);
				if (item11 >= num11 && item11 <= num12)
				{
					arg = true;
				}
			}
			...
			pushVisited.Clear();
			num10 = PlaceTriggerWithPush(num9, item11, item9, arg);
			if (num10 >= 0)
			{
				if (num10 != item11)
				{
					list5.Add($"TRIGGER Sprite 0x{item9:X2} at TMX({item12},{item13}) → Placed at ({num9},{num10})");
				}
				continue;
			}
			bool flag6 = false;
			for (int num13 = 0; num13 <= maxPlayableRow; num13++)
			{
				int num14 = num13 * width + num9;
				if (sprites[num14] == -1)
				{
					sprites[num14] = item9;
					list5.Add($"TRIGGER Sprite 0x{item9:X2} at TMX({item12},{item13}) → Placed at ({num9},{num13}) [fallback]");
					flag6 = true;
					break;
				}
			}
			...
			for (int num15 = 1; num15 <= 2; num15++)
			{
				...
				{
					if (num17 < 0 || num17 >= width)
					{
						continue;
					}
					for (int num18 = 0; num18 <= maxPlayableRow; num18++)
					{
						int num19 = num18 * width + num17;
						if (sprites[num19] == -1)
						{
							sprites[num19] = item9;
							list5.Add($"TRIGGER Sprite 0x{item9:X2} at TMX({item12},{item13}) → Placed at ({num17},{num18}) [adjacent col fallback]");
							flag7 = true;
							break;
						}
					}
			...
			if (!flag7)
			{
				list5.Add($"TRIGGER Sprite 0x{item9:X2} at TMX({item12},{item13}) → DROPPED (column {num9} full, no adjacent space)");
			}
		}
		XElement xElement4 = xElement.Elements("imagelayer").FirstOrDefault(delegate(XElement l)
		{
			string? text6 = (string?)l.Attribute("name");
			return (text6 != null && text6.Contains("parallax", StringComparison.OrdinalIgnoreCase)) || (((string?)l.Attribute("name"))?.Contains("Image Layer 1") ?? false);
		});
		...
		{
			string? text6 = (string?)l.Attribute("name");
			return (text6 != null && text6.Contains("ground", StringComparison.OrdinalIgnoreCase)) || (((string?)l.Attribute("name"))?.Contains("Image Layer 2") ?? false);
		});
		...
		return new TmxLevel
		{
			...
			LoadCollisionMessages = ((list5.Count > 0) ? string.Join("\n", list5) : null),

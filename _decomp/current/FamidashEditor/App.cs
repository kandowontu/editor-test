...
public class App : Application
{
	...
	private void DetectAndForceSoftwareRenderingIfNeeded()
	{
		try
		{
			...
			try
			{
				ProcessStartInfo startInfo = new ProcessStartInfo("wmic", "path win32_VideoController get Name,AdapterCompatibility /format:csv")
				{
					CreateNoWindow = true,
	...
	private static void LogException(Exception? ex, string source)
	{
		try
		{
			string path = Path.Combine(AppDomain.CurrentDomain.BaseDirectory ?? ".", "sim-crash.log");

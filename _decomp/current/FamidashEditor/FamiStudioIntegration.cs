...
public class FamiStudioIntegration
{
		...
		try
		{
			...
			if (obj7 is IEnumerable enumerable && !(obj7 is string))
			{
				...
				foreach (object item in enumerable)
				{
					...
					IEnumerable<PropertyInfo> enumerable2 = from pp in type4.GetProperties()
	...
	public List<string> ParseFamiStudioTextExport(string path)
	{
		...
		for (int i = 0; i < array.Length; i++)
		{
			...
			if (!text2.StartsWith("Song", StringComparison.OrdinalIgnoreCase) || (text2.Length != 4 && !char.IsWhiteSpace(text2[4]) && text2[4] != '\t'))
			{
				continue;
			}
			string text3 = null;
			for (int j = i; j < Math.Min(array.Length, i + 24); j++)
			{
				...
				if (j > i && text4.Length > 0 && !char.IsWhiteSpace(text4[0]))
				{
					break;
				}
				...
				if (match.Success)
				{
					text3 = match.Groups[1].Value.Trim();
	...
	public List<string> TryParseFmsSongNames(string fmsPath)
	{
		...
		try
		{
			...
			MatchCollection matchCollection = Regex.Matches(text, "\"name\"\\s*:\\s*\"([^\"]+)\"", RegexOptions.IgnoreCase);
			...
			if (list.Count == 0)
			{
				MatchCollection matchCollection2 = Regex.Matches(text, "Song\\s*[:=]\\s*\"?([A-Za-z0-9 _-]{1,60})\"?", RegexOptions.IgnoreCase);
	...
	public void PlayTrack(string fmsPath, int trackIndex)
	{
		lock (playLock)
		{
			...
			ProcessStartInfo startInfo = new ProcessStartInfo(text2, arguments)
			{
				CreateNoWindow = true,
				UseShellExecute = false,
				RedirectStandardOutput = true,
		...
		try
		{
			if (Math.Abs(playbackRate - 1.0) > 0.0001 && reader != null)
			{
				try
				{
					...
					VarispeedSampleProvider sourceProvider = new VarispeedSampleProvider(source, playbackRate);
					SampleToWaveProvider16 waveProvider = new SampleToWaveProvider16(sourceProvider);
	...
	private string GetMusicCacheDir()
	{
		try
		{
			...
			string text = Path.Combine(folderPath, "Famidash Editor", "Music");
	...
	private string GetCachedMusicPath(string fmsPath, int trackIndex)
	{
		try
		{
			...
			string value = BitConverter.ToString(array).Replace("-", "").ToLowerInvariant();
	...
	private string EnsureCachedMusic(string fmsPath, int trackIndex)
	{
		...
		string text3 = ConvertWavToCached(text2, cachedMusicPath);
	...
	public void WarmAndPrime(string fmsPath)
	{
		try
		{
			...
			try
			{
				List<string> list = EnumerateTracks(fmsPath);
			}
			catch
			{
			}
			...
			foreach (Assembly assembly in alc.Assemblies)
			{
				...
				List<MethodInfo> list2 = (from mi in type.GetMethods(BindingFlags.Instance | BindingFlags.Static | BindingFlags.Public)

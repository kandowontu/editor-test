...
public static class PfTrace
{
	...
	public static void State(string tag, int xFx, int yFx, int vxFx, int vyFx, int slopeFrames, int slopeWasOn, int slopeType, int lastSlopeType, bool input, bool grav, bool mini, int gameMode, int camYFx = 0, bool onGround = false, bool dashing = false)
	{
		if (ShouldEmit())
		{
			...
			string body = $"X={xFx >> 8}.{xFx & 0xFF:X2} Y={yFx >> 8}.{yFx & 0xFF:X2} Ypx={value} Vx={vxFx} Vy={vyFx} sFr={slopeFrames} swOn={slopeWasOn} sT={slopeType} lst={lastSlopeType} inp={(input ? 1 : 0)} grav={(grav ? 1 : 0)} mini={(mini ? 1 : 0)} gm={gameMode} onG={(onGround ? 1 : 0)} dash={(dashing ? 1 : 0)} camY={camYFx >> 8}.{camYFx & 0xFF:X2}";
			Event(tag, body);
		}
	}
	...
	public static void Branch(string tag, string condition, bool taken, string? extra = null)
	{
		if (ShouldEmit())
		{
			string text = $"cond=\"{condition}\" taken={(taken ? 1 : 0)}";
			if (extra != null)
			{
				text = text + " " + extra;
			}

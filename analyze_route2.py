...
pr("=== TERRAIN DETAIL: cols 1828-1850, rows 18-27 ===")
pr(f"{'row':>3s} {'wY':>4s} |", end='')
...
pr()
pr("=== SPRITES: cols 1828-1850, rows 18-27 ===")
for tmx_row in range(18, 28):
    wY = (tmx_row - grr) * 16
...
pr()
pr("=== FORWARD COLLISION ANALYSIS AT COL 1847 ===")
col = 1847
pr(f"Forward collision probe at rightEdge in col {col}")
pr(f"For non-mini cube: hbW=12, hbH=12, hbOffY=0")
pr(f"centerY = playerY + 6")
pr()
for playerY in range(270, 340, 2):
    ...
    pr(f'  Y={playerY:3d} cY={centerY:3d} tileY={tileY:2d} row={tileArrayY:2d} ed={ed:3d} ({name:10s}) localY={localY:2d} -> {result}')

pr()
pr("=== FORWARD COLLISION ANALYSIS AT COL 1833 ===")
col = 1833

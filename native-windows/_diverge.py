...
print("First sustained divergence runs (|dy|>4 for >2 frames)")
prev_dy=0; run_start=-1; run_dy_max=0
for sf in sorted(set(nes.keys())&set(pf.keys())):
    ...
    if abs(dy) > 4:
        ...
        if run_start>=0 and sf-run_start>=2:
            print(f"  run sf={run_start}-{sf-1} ({sf-run_start} frames) max_dy={run_dy_max:+3d} gm_at_start={nes[run_start][14]}")
        run_start=-1

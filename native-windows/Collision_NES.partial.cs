...
#pragma warning restore CS0414
        ...
        {
            // NES bg_coll_U probe (collision.h:921, post-edit):
            //   temp_y = Generic.y + (mini ? (0x10-h)>>1 : 0) + (wave/snake ? 0 : 1)
            // For wave/snake the trailing +1 is SUPPRESSED; we are in the wave path so use Generic.y directly
            // (plus mini-centering when mini).  Previously this was Generic_y - 1, off by 1 vs NES.
            temp_x = Generic_x + 4;
            int miniCenterAdj = (currplayer_mini != 0) ? ((0x10 - Generic_height) >> 1) : 0;
            temp_y = Generic_y + miniCenterAdj;

...
namespace FamidashEditor
{
    ...
    public class PathfinderEngine
    {
                                       ...
                                       int startGameMode, bool startGravFlipped, bool startMini)
        {
            ...
            PfLog($"[RUN_STATE] X_fixed=0x{state.X_fixed:X4} Y_fixed=0x{state.Y_fixed:X4} VelX=0x{state.VelX_fixed:X4} VelY=0x{state.VelY_fixed:X4} gravMul={state.GravMul} onGround={state.OnGround}");
        ...
        private long BfsQuantizeKey(ref SimState s)
        {
            ...
            // Pack game mode, gravity, mini, onGround, and prior-input-held.
            // Prior-input-held matters for press-gated robot jump starts.
            int flags = (s.GameMode & 0x7) | ((s.GravFlipped ? 1 : 0) << 3)
                      | ((s.Mini ? 1 : 0) << 4) | ((s.OnGround ? 1 : 0) << 5)
                      | ((s.PrevInputHeld ? 1 : 0) << 6);
                            ...
                            int startGameMode, bool startGravFlipped, bool startMini)
        {
            ...
            try
            {
                ...
                for (int frame = 0; frame < MAX_FRAMES && frontier.Count > 0; frame++)
                {
                    ...
                    // Diversity slots: 25% from underrepresented Y bins + gravity diversity
        ...
        private bool StepFrame(ref SimState s, bool input, out bool endLevel)
        {
            ...
            else if (s.GameMode == 2) // Ball mode
            {
                ...
                //    - buffer>0 & grounded & !switched ? flip (buffered landing)
                {
                    ...
                    else if (s.BallInputBuffer > 0 && s.BallFlipCooldown == 0 && BallIsGrounded(ref s))
                    {
                        shouldFlip = true;
                    }
        ...
        private void BallGravityStep(ref SimState s)
        {
            ...
            SharedPhysics.CommonGravityRoutine(
                ...
                s.Dashing, 1.0, 1.0, true, s.VelX_fixed,
                clampMaxY);
        ...
        private void SpiderScanUp(ref SimState s)
        {
            ...
            // Final ceiling eject
            int finalHbH = s.Mini ? 8 : 15;
            var (c2, e2) = BgCollU_Spider(playerX_px, scanY, hbW, finalHbH);
            if (c2) s.Y_fixed = (scanY - e2) << 8; // Note: eject is negative for ceiling
        }
        ...
        private void SpiderScanDown(ref SimState s)
        {
            ...
            // Final floor eject
            var (c2, e2) = BgCollD_Spider(playerX_px, scanY + hbOffY, hbW, hbH);
            if (c2) s.Y_fixed = (scanY - e2) << 8;
        }
        ...
        private void ShipEject(ref SimState s, out bool died)
        {
            ...
            // NES ufo_ship_eject: ceiling check, NO velocity guard, NO spike death.
            var (ceilHit, ceilBotY, _) = CheckCeiling(collX, collY, hbW, hbH);
            if (ceilHit)
            {
                ...
                s.Y_fixed = newY << 8;
                s.VelY_fixed = 0;
                // NOTE: do NOT update collY here � SIM's UfoShipEject_Fresh uses
                // a stale local collisionY for the floor check (matching NES which
                // sets Generic.y once before the eject and doesn't refresh it after
                // the ceiling snap).  The floor CheckCollisionDown must see the
                // pre-ceiling-eject coordinates.
            }
        ...
        private void ApplySpiderTeleport(ref SimState s, bool goUp)
        {
            ...
            if (goUp)
            {
                ...
                // Post-eject from ceiling
                int finalY = s.Y_fixed >> 8;
                var (collFin, ejectFin) = BgCollU_Spider(playerX_px, finalY, hbW, hbH);
                if (collFin)
                    s.Y_fixed -= (ejectFin << 8);
                s.VelY_fixed = 0;
            }
            else
            {
                ...
                // Post-eject from floor
                int finalY = s.Y_fixed >> 8;
                hbOffY = s.Mini ? ((0x10 - hbH) >> 1) : 0;
                var (collFin, ejectFin) = BgCollD_Spider(playerX_px, finalY + hbOffY, hbW, hbH);
                if (collFin)
                    s.Y_fixed -= (ejectFin << 8);
                s.VelY_fixed = 0;
            }

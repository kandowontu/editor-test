...
namespace FamidashEditor
{
    ...
    private string? portalDebugPath = null;
        ...
        private void UpdateParallaxTint()
        {
            ...
            parallaxTonedImages = ColorHelpers.CreateRgbReplacedImages(parallaxImages, backgroundTint);
        ...
        private void UpdateGroundTint()
        {
            ...
            groundTonedImages = ColorHelpers.CreateHueShiftedImages(groundImages, groundTint);
        ...
        private void UpdateTileTint()
        {
            ...
            tileTonedImages = ColorHelpers.CreateHslShiftedImages(tileImages, tileTint);

            // Also tint the animated saw frames using HSL hue shift
            sawFrame1TilesTinted = ColorHelpers.CreateHslShiftedImages(sawFrame1Tiles, tileTint);
            sawFrame2TilesTinted = ColorHelpers.CreateHslShiftedImages(sawFrame2Tiles, tileTint);
            smallSawFrame1TilesTinted = ColorHelpers.CreateHslShiftedImages(smallSawFrame1Tiles, tileTint);
            smallSawFrame2TilesTinted = ColorHelpers.CreateHslShiftedImages(smallSawFrame2Tiles, tileTint);
            largeSawFrame1TilesTinted = ColorHelpers.CreateHslShiftedImages(largeSawFrame1Tiles, tileTint);
            largeSawFrame2TilesTinted = ColorHelpers.CreateHslShiftedImages(largeSawFrame2Tiles, tileTint);
        ...
        private void PopulateTilesPanel()
        {
            ...
            foreach (int idx in displayOrder)
            {
                ...
                if (isPlayerTile)
                {
                    ...
                    {
                        paletteSrc = ColorHelpers.CreatePlayerReplacedFromBaseAndToned(baseSrc, tonedSrc ?? baseSrc, playerTint);
                    }
                    else
                    {
        ...
        private BitmapSource? GetOrCreateSingleTinted(BitmapSource src, Color tint, bool useRgbReplace, int scaleKey)
        {
            try
            {
                ...
                if (useRgbReplace)
                {
                    arr = ColorHelpers.CreateRgbReplacedImages(new ImageSource[] { src }, tint);
                }
                else
                {
                    arr = ColorHelpers.CreateHueShiftedImages(new ImageSource[] { src }, tint);
                }
            ...
            try
            {
                ...
                else if (tileIdx >= 0 && tileIdx < 256)
                {
                    ...
                    if (IsPlayerReplacementTile(tileIdx) && playerTintEnabled)
                    {
                        ...
                        src = ColorHelpers.CreatePlayerReplacedFromBaseAndToned(baseSrc, tonedSrc ?? baseSrc, playerTint);
                    }
                    else
                    {

...
try {
    ...
    $publishArgs = @("publish", "-c", "Release", "-o", $OutputDir)
    if ($NoDebugLogging) {
        Write-Host "Building WITHOUT debug logging..." -ForegroundColor Yellow
        $publishArgs += "/p:DefineConstants=DISABLE_DEBUG_LOGGING"
    }
    dotnet @publishArgs
    $sw.Stop()

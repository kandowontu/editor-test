...
for L in pf[1:]:
    p=L.split(',')
    pfs[int(p[0])]=p
print("sim |  NES_y NES_vy a gm grav | PF_y PF_vy onG mode grv mini | dy dvy")
for sf in range(3160, 3200):
    if sf not in nes or sf not in pfs: continue
    n=nes[sf]; pf_=pfs[sf]
    nesy=int(n[3]); nesvy=int(n[10])
    pfy=int(pf_[7]); pfvy=s32(pf_[3])
    print(f"{sf:4d} | {nesy:5d} {nesvy:6d} {n[4]} {n[14]:2s} {n[12]:1s} | {pfy:4d} {pfvy:6d} {pf_[8]} {pf_[11]:2s} {pf_[12]:1s} {pf_[23]:1s} | {nesy-pfy:+3d} {nesvy-pfvy:+5d}")

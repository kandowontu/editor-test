import csv
from pathlib import Path
pf=Path(r"C:\Users\kando\AppData\Local\Temp\famidash_pf_trace_untitled_20260517_131605_527.csv")
mes=Path(r"C:\Users\kando\AppData\Local\Temp\famidash_mesen_trace_silentcircles_20260517_143603_468.csv")
off=13
pf_rows={int(r['frame']):r for r in csv.DictReader(pf.open(newline=''))}
lines=mes.read_text(errors='replace').splitlines(); hdr=lines[1].split(',')
mes_rows={}
for ln in lines[2:]:
 p=ln.split(',')
 if len(p)>=len(hdr) and p[0].isdigit():
  d={hdr[i]:p[i] for i in range(len(hdr))}
  mes_rows[int(d['rom_frame'])]=d
first_any=None; first_1px=None; first_mode_mis=None
for f in sorted(pf_rows):
 pr=pf_rows[f]
 if int(pr['mode'])!=6: continue
 mr=mes_rows.get(f+off)
 if not mr: continue
 if int(mr['gamemode'])!=6:
  if first_mode_mis is None: first_mode_mis=(f,f+off,int(pr['mode']),int(mr['gamemode']),int(pr['X_px']),int(mr['px']),int(pr['Y_px']),int(mr['py']))
  continue
 py=int(pr['Y_px']); my=int(mr['py'])
 pv=int(pr['VelY_fixed'],16)&0xFFFF; mv=int(mr['vel_y'])&0xFFFF
 if py!=my or pv!=mv or int(pr['X_px'])!=int(mr['px']):
  if first_any is None: first_any=(f,f+off,int(pr['X_px']),int(mr['px']),py,my,pv,mv)
  if first_1px is None and abs(py-my)==1 and pv==mv and int(pr['X_px'])==int(mr['px']):
   first_1px=(f,f+off,int(pr['X_px']),py,my,pv)
print('first_any_wave_mismatch',first_any)
print('first_1px_wave_mismatch',first_1px)
print('first_wave_mode_mismatch',first_mode_mis)

...
public class FamiStudioIntegration
{
	...
	public List<string> EnumerateTracks(string fmsPath)
	{
		...
		try
		{
			...
			if (propertyInfo3 != null && propertyInfo3.PropertyType == typeof(int))
			{
				...
				List<string> list3 = (from i in Enumerable.Range(0, count)
					select $"Song {i}").ToList();
	...
	public List<string> TryParseFmsSongNames(string fmsPath)
	{
		...
		try
		{
			...
			foreach (Match item in Regex.Matches(@string, "\"name\"\\s*:\\s*\"([^\"]+)\"", RegexOptions.IgnoreCase))
			{
			...
			if (list.Count == 0)
			{
				foreach (Match item2 in Regex.Matches(@string, "Song\\s*[:=]\\s*\"?([A-Za-z0-9 _-]{1,60})\"?", RegexOptions.IgnoreCase))
				{
					string text2 = item2.Groups[1].Value.Trim();
					if (!string.IsNullOrEmpty(text2) && !list.Contains(text2))
					{
						list.Add(text2);
					}

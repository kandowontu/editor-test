...
public static class TmxHandler
{
	...
	public static TmxLevel LoadTmx(string filePath, bool useLegacyTriggerOffset = false)
	{
		...
		foreach (XElement item14 in list)
		{
			...
			string a2 = (string?)item14.Attribute("name");
			...
			if (string.Equals(a2, "famidash", StringComparison.OrdinalIgnoreCase))
			{
		...
		foreach (XElement item15 in xElement.Elements("layer").ToList())
		{
			...
			if ((string?)xElement3.Attribute("encoding") == "csv")
			{
				...
				for (int i = 0; i < Math.Min(array2.Length, num); i++)
				{
					int num2 = array2[i];
					if (num2 <= 0)
					{
					...
					{
						if (num2 < item16 || num2 >= item16 + 256)
						{
							continue;
						}
						int num3 = num2 - item16;
						int num4 = i / width;
						int num5 = i % width;
						int item = num5;
						int item2 = num4;
						bool flag2 = !useLegacyTriggerOffset && IsTriggerSprite(num3);
						if (flag2)
						{
							num5 -= 10;
							if (num5 < 0)
						...
						list5.Add((num3, num5, num4, item, item2, flag2));
						flag = true;
						break;
					}
					if (flag)
					...
					{
						if (num2 >= item17 && num2 < item17 + 256)
						{
							array[i] = num2 - item17;
							flag = true;
							break;
		...
		list6.Sort(delegate((int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) a, (int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) b)
		{
			int num34 = a.y.CompareTo(b.y);
			return (num34 != 0) ? num34 : a.x.CompareTo(b.x);
		});
		list7.Sort(delegate((int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) a, (int spriteIdx, int x, int y, int originalX, int originalY, bool isTrigger) b)
		{
			int num33 = a.x.CompareTo(b.x);
			return (num33 != 0) ? num33 : a.y.CompareTo(b.y);
		});
		foreach (var item18 in list6)
		{
			...
			if (sprites[num6] != -1)
			{
				...
				bool num8 = IsInteractiveSprite(num7);
				bool flag3 = IsInteractiveSprite(item3);
				if (num8 && !flag3)
				{
					list4.Add($"NORMAL Sprite 0x{item3:X2} (deco) at TMX({item6},{item7}) SKIPPED — would overwrite interactive sprite 0x{num7:X2} at ({item4},{item5})");
					continue;
				}
				list4.Add($"NORMAL Sprite 0x{item3:X2} at TMX({item6},{item7}) OVERWRITES existing sprite 0x{num7:X2} at ({item4},{item5})");
			}
			sprites[num6] = item3;
		}
		...
		{
			int num16 = Math.Max(0, Math.Min(maxPlayableRow, row));
			int num17 = num16 * width + col;
			if (sprites[num17] == -1)
			{
				sprites[num17] = spriteId;
				return num16;
			}
			if (!IsTriggerSprite(sprites[num17]))
			{
				for (int num18 = 0; num18 <= maxPlayableRow; num18++)
				{
					...
					{
						int num19 = row - num18;
						if (num19 >= 0 && num19 <= maxPlayableRow)
						{
							int num20 = num19 * width + col;
							if (sprites[num20] == -1)
							{
								sprites[num20] = spriteId;
								return num19;
							}
						}
						if (num18 != 0)
						{
							int num21 = row + num18;
							if (num21 >= 0 && num21 <= maxPlayableRow)
							{
			...
			int item13 = (col << 16) | num16;
			if (pushVisited.Contains(item13))
			{
				return -1;
			}
			pushVisited.Add(item13);
			int arg2 = sprites[num17];
			if (preferUpFirst)
			{
				for (int num27 = 1; num27 <= maxPlayableRow; num27++)
				{
					int num28 = num16 - num27;
					if (num28 >= 0 && PlaceTriggerWithPush(col, num28, arg2, preferUpFirst) >= 0)
					{
						sprites[num17] = spriteId;
						pushVisited.Remove(item13);
						return num16;
					}
					int num29 = num16 + num27;
					if (num29 <= maxPlayableRow && PlaceTriggerWithPush(col, num29, arg2, preferUpFirst) >= 0)
					{
						sprites[num17] = spriteId;
						pushVisited.Remove(item13);
						return num16;
			...
			{
				for (int num30 = 1; num30 <= maxPlayableRow; num30++)
				{
					int num31 = num16 + num30;
					if (num31 <= maxPlayableRow && PlaceTriggerWithPush(col, num31, arg2, preferUpFirst) >= 0)
					{
						sprites[num17] = spriteId;
						pushVisited.Remove(item13);
						return num16;
					...
					if (num32 >= 0 && PlaceTriggerWithPush(col, num32, arg2, preferUpFirst) >= 0)
					{
						sprites[num17] = spriteId;
						pushVisited.Remove(item13);
						return num16;
		...
		foreach (var item19 in list7)
		{
			...
			int num9 = Math.Max(0, Math.Min(width - 1, item9));
			int num10 = -1;
			bool arg = false;
			try
			{
			...
			pushVisited.Clear();
			num10 = PlaceTriggerWithPush(num9, item10, item8, arg);
			if (num10 >= 0)
			{
				if (num10 != item10)
				{
					list4.Add($"TRIGGER Sprite 0x{item8:X2} at TMX({item11},{item12}) → Placed at ({num9},{num10})");
				}
				continue;
			}
			bool flag4 = false;
			for (int j = 0; j <= maxPlayableRow; j++)
			{
				int num13 = j * width + num9;
				if (sprites[num13] == -1)
				{
					sprites[num13] = item8;
					list4.Add($"TRIGGER Sprite 0x{item8:X2} at TMX({item11},{item12}) → Placed at ({num9},{j}) [fallback]");
					flag4 = true;
					break;
				}
			}
			...
			for (int k = 1; k <= 2; k++)
			{
				...
				foreach (int num14 in array3)
				{
					if (num14 < 0 || num14 >= width)
					{
						continue;
					}
					for (int n = 0; n <= maxPlayableRow; n++)
					{
						int num15 = n * width + num14;
						if (sprites[num15] == -1)
						{
							sprites[num15] = item8;
							list4.Add($"TRIGGER Sprite 0x{item8:X2} at TMX({item11},{item12}) → Placed at ({num14},{n}) [adjacent col fallback]");
							flag5 = true;
							break;
						}
					}
			...
			if (!flag5)
			{
				list4.Add($"TRIGGER Sprite 0x{item8:X2} at TMX({item11},{item12}) → DROPPED (column {num9} full, no adjacent space)");
			}
		}
		XElement xElement4 = xElement.Elements("imagelayer").FirstOrDefault(delegate(XElement l)
		{
			string? text5 = (string?)l.Attribute("name");
			return (text5 != null && text5.Contains("parallax", StringComparison.OrdinalIgnoreCase)) || (((string?)l.Attribute("name"))?.Contains("Image Layer 1") ?? false);
		});
		...
		if (xElement4 != null)
		{
			parallaxX = ((double?)xElement4.Attribute("parallaxx")).GetValueOrDefault(1.0);
			parallaxY = ((double?)xElement4.Attribute("parallaxy")).GetValueOrDefault(1.0);
			parallaxRepeatX = ((int?)xElement4.Attribute("repeatx")).GetValueOrDefault() == 1;
			parallaxRepeatY = ((int?)xElement4.Attribute("repeaty")).GetValueOrDefault() == 1;
		...
		if (xElement6 != null)
		{
			...
			groundRepeatX = ((int?)xElement6.Attribute("repeatx")).GetValueOrDefault() == 1;
		...
		if (xElement8 != null)
		{
			...
			if (xElement9 != null)
			{
				chunkWidth = ((int?)xElement9.Attribute("width")).GetValueOrDefault(16);
				chunkHeight = ((int?)xElement9.Attribute("height")).GetValueOrDefault(valueOrDefault);
			}
